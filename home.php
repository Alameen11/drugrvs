...
}else if(isset($_GET['apr'])){
	$id = $_GET['apr'];
	$sql = mysqli_query($link, "UPDATE products SET status='APPROVED' WHERE id='$id'");
}else if(isset($_GET['sus'])){
	$id = $_GET['sus'];
	$sql = mysqli_query($link, "UPDATE products SET status='SUSPENDED' WHERE id='$id'");
}else if(isset($_GET['pro'])){
	$id = $_GET['pro'];
	$sql = mysqli_query($link, "UPDATE products SET status='PROCESSING' WHERE id='$id'");
}
	...
	<body>
	   <img src="nafdac.jpg"
		...
		<div class="icon">
			...
			<h2 style="font-family: 'Trebuchet MS', sans-serif;">REGISTERED PRODUCTS</h2>
			...
			<table style="text-align: left; padding: 3px; font-family: 'Trebuchet MS', sans-serif;">
				...
				$cn = 1;
				$sql = mysqli_query($link, "SELECT * FROM products ORDER BY name ASC");
				...
				<tr>
					...
					<th><?php echo $rows['distributor']; ?></th>
					<th><?php echo date("d.m.Y h:i A", $rows['datex']); ?></th>
					<th><?php echo $rows['nafdacno']; ?></th>
					<th><?php echo $rows['phone']; ?></th>
					<th><?php echo $rows['status']; ?></th>
					<th><?php if($rows['nextasses'] != "-"){ echo date("d.m.Y", $rows['nextasses']); } ?></th>
					<th><?php echo "<a class='button-25' href='home.php?apr=".$rows['id']."'>APPROVE</a>"; ?></th>
					<th><?php echo "<a class='button-25' href='home.php?sus=".$rows['id']."'>SUSPEND</a>"; ?></th>
					<th><?php echo "<a class='button-25' href='home.php?pro=".$rows['id']."'>PROCESSING</a>"; ?></th>				
				</tr>
				<?php 
					$cn = $cn + 1;
					} 
